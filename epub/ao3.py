...
def a_body(url): 
    ...
    body = soup.find('div',{'id':'workskin'})
    return body
...
def a_gettitle(urL): #this just gives the title of the fic not of the specific page.
    page = requests.get(urL)
    soup = BeautifulSoup(page.content, "html.parser")
...
def a_getchaptertitle(url):
    ...
    title_text_raw = title.get_text()
    title_text_split = title_text_raw.split('-')
    return title_text_split[1].strip()
